...
public class JMeterCommandUtilTest {
    ...
    public void testClientCommand() {
        assertThat(
            ...
            is(
            "PATH=$PATH:/apache-jmeter-3.2/bin JVM_ARGS=\"-Xms384m -Xmx384m\" " +
                "nohup jmeter -n -t IPIFY.jmx -l log_remote.jtl -R1.1.1.1,2.2.2.2 " +
                "-Djava.rmi.server.hostname=3.3.3.3 -Dclient.rmi.localport=4040 -Dserver.rmi.localport=4040 " +
                "> /tmp/jmeter-client.log 2>&1 &"
            )
        );
    }
    ...
    public void testServerCommand() {
        assertThat(
            ...
            is(
            "PATH=$PATH:/apache-jmeter-3.2/bin JVM_ARGS=\"-Xms384m -Xmx384m\" " +
                "nohup jmeter-server -Djava.rmi.server.hostname=3.3.3.3 " +
                "-Dclient.rmi.localport=4040 -Dserver.rmi.localport=4040 > /tmp/jmeter-server.log 2>&1 &"
            )
        );
    }
}
